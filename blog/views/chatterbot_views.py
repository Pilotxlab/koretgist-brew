...
from blog.chats.functions import(
    master_post_fb,
    greet_post_fb,
    post_weather_fb,
    askLocation,
                    ...
                    if 'text' in message['message']:
                        ...
                        if type(message['message']['text']) == bytes:
                            message['message']['text'] = str(
                                message['message']['text'], "utf-8", errors="ignore")
                        else:
                            message['message']['text'] = str(
                        ...
                        if 'coordinates' in message['message']['attachments'][0]['payload']:
                            ...
                            getWeather(location['lat'],
                                       location['long'], fb_id)

                        else:
                            text = "You liked me"
                            master_post_fb(fb_id, text)
                            # so that api call just happens once and then it exits.
                            break
...
def send(request, response):
    ...
    user_details_params = {'fields': 'first_name,last_name,profile_pic',
                           'access_token': 'EAAXsQKdeWhYBANEnYdJxrkGclTfc7MaZCY4H6qrEoPttwn2t73zF3i6SsN5Q3FweR1w1v6QqyFVmchEWwnSHgELwKx70HTAulZAmEBG1rE2hsz0gtwqcZBCVhnsl2CdQz1jjn8kUmZBeUBj1I9bPimAlZBUzON9rvKcakaglp0AZDZD'}
    user_details = requests.get(user_details_url, user_details_params).json()
    pprint(user_details)
    if request['entities']:
        text = str(response['text'])
        text = cleaning_text(text)
        # Storing intent type for user interactions for different intents.
        intent_type = request['entities']['intent'][0]['value']
        if intent_type == 'greet':
            text = "Hey " + str(user_details['first_name']) + \
                " This is my beta version,I can only find weather condition,but soon I will try to learn more things to become more useful for you :-) :)"
...
client = Wit(access_token="H6TJ5STFJQEPVI3YTEJDB5QL4TJ2FLVV")
